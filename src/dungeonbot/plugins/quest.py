"""Define logic for the Quest Log plugin."""
...
usage:
    ...
    !quest detail <INTEGER or CASE INSENSITIVE STRING>
    !quest edit <INTEGER> <FIELD NAME> [CASE SENSITIVE STRING]
    !quest update <INTEGER> <CASE SENSITIVE STRING>
    !quest complete <INTEGER or CASE INSENSITIVE STRING>

    Valid FIELD NAMEs:
        title | description | quest_giver | location
...
```"""
    ...
    def run(self):
        ...
        Handle valid sub-commands and display appropriate quest info.
    ...
    def get_detail(self, *args):
        """Get information for a single quest given its title or ID."""
        try:
            quest = self._get_id_or_name(args)

        except IndexError:
            return (
                "Must supply either a quest title or a quest ID. " +
                "Try '!help quest' for examples on usage."
            )
    ...
    def update_quest(self, *args):
        ...
        if quest:
            ...
            if desc:
                ...
                output = "*{}* description updated.".format(
                    quest.title.title()
                )
                output += "\n\n-----------------------\n\n"
    ...
    def complete_quest(self, *args):
        ...
        try:
            ...
            return (
                "Must either a quest title or a quest ID. " +
                "Try '!help quest' for examples on usage."
            )

        if quest and quest.status:
            QuestModel.complete(quest.id)
            return "Congratulations on completing {}!".format(
                quest.title.title()
            )
    ...
    def edit_quest(self, *args):
        ...
        if quest:
            ...
            try:
                ...
                return (
                    'You need to supply a field to be edited: ' +
                    '[{}]'.format("|".join(white_list))
                )

            if this_field not in white_list:
                return (
                    'That is not a field you can edit. Try again: ' +
                    '[{}}]'.format("|".join(white_list))
                )
            ...
            if not new_val:
                return 'You must provide a value for the "{}" field.'.format(
                    this_field
                )
    ...
    def _invalid_input(self):
        ...
        return (
            ...
            '"!help quest" for a list of valid arguments.'
        )
    ...
    def _get_id_or_name(self, args):
        """Use the quest ID or name to retrieve from the database."""
        try:
            the_id = int(args[1])
    ...
    def _slack_msg(self, quest):
        ...
        output.append("_Date Added: {}_".format(
            quest.created.strftime(TIME_FMT))
        )

        output.append("_Last Updated: {}_".format(
            quest.last_updated.strftime(TIME_FMT))
        )

        if quest.status:
            output.append("Status: Active")
        else:
            output.append("Status: Inactive | Completed: {}".format(
                quest.completed_date.strftime(TIME_FMT))
            )
